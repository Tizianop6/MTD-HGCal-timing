...
OutputDir = "/eos/user/t/tipaulet/www/track_res_Energy/cfrRegPtRegE/"
...
def ReadFileAndFit(filename,return_dict,key,tag="prova",eta=1.9, maxfiles=1):
    ...
    for i in range(len(dumperInputV5.inputReaders)):
        ...
        for ev in range(len(tms)):
            ...
            for simCand_idx in range(len(tsSCEv.barycenter_x)):
                ...
                simRegrE = tsSCEv.regressed_energy[simCand_idx]
                ...
                residual=(simRegrE-tracksEv.track_pt[tk_idx_in_coll]*math.cosh(eta))-(simRegrPt-tracksEv.track_pt[tk_idx_in_coll])*math.cosh(eta)
    ...
    plt.legend()
    plt.xlabel(("Energy residual [GeV]"))
    plt.ylabel("Entries")
