...
OutputDir = "/eos/user/t/tipaulet/www/PtResolution/"
...
def ReadFileAndFit(filename,return_dict,key,tag="prova",eta=1.9, maxfiles=1):
    ...
    rng=0.1
    y_h,x_h,_=plt.hist(residual_list, range=(-rng+1,+rng+1), bins = 100, histtype = "step", lw = 2, color = "xkcd:magenta", label="Combination")
    ...
    xlist=np.linspace(-rng+1,rng+1,1000)

    hlist=[]
    hlist.append(hist.Hist(hist.axis.Regular(100,-rng+1,rng+1,name="tres"), name="name", label="label"))
...
def PlotRes(dict_resolutions,tag="test",eta=1.9):
    ...
    for enkey in dict_resolutions.keys():
        ...
        pts["y2"].append(dict_resolutions[enkey][0]/(enkey*math.cosh(eta)))
        pts["y2Err"].append(dict_resolutions[enkey][1]/(enkey*math.cosh(eta)))
    ...
    plt.savefig(OutputDir + tag+"eta"+str(eta)+"SigmaEdEvspT.png")
