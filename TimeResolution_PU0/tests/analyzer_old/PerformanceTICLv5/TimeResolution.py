...
pTBinsText = []
inputPaths = ["/eos/user/t/tipaulet/Local_Energy_Samples/"#,
...
for ih, (histoDir, label) in enumerate(zip(histoDirs, labels)):
    ...
    for etaText in etaBinsText:
        ...
        folderPrefix = "SinglePionTiming_"+ etaText + "_50GeV"
        ...
        #histo = make_scOrTsOverCP_energy_histogram(name=f"tsOverCP_energy{label}", label=f"Trackster Merged - {label} / CaloParticle energy")
        histo = make_tres_histo(name=f"tsOverCP_energy{label}", label=f"Trackster Merged - {label} / CaloParticle energy")
        print(df)
        print(df.columns)
        fill_tres_histo(histo, df)
        #fill_scOverCP_energy_histogram(histo, df, minE, maxE)
        histos.append(histo)
        histo.plot1d()
        plt.savefig(f"Plots/{folderPrefix}")
        plt.clf()
        histo_fit = fitMultiHistogram(histos)
        #histo_fit[0][0].plot()
        #plt.savefig(f"Plots/{folderPrefix}_fit")
        #plt.clf()
...
OutputDir = "/eos/user/t/tipaulet/www/HGCAL/TICLv5PerformancePostFix/Resolution/FixSkeletonsCloseByPion200PU/"
